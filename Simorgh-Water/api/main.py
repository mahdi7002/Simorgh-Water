"""
API و داشبورد حرفه‌ای سیستم مدیریت چرخه آب سیمرغ
الهام‌گرفته از داشبوردهای Xylem Vue، Bentley WaterSight و Schneider EcoStruxure
"""
...
from core.models import HolisticViewResponse
...
@app.get("/api/health")
async def health():
    return {"status": "ok", "app": settings.APP_NAME, "version": settings.APP_VERSION, "mode": "mock" if settings.MOCK_MODE else "live"}
...
async def api_holistic(user_id: str = Query(default="USER-YZD-001")):
    ...
    return {
        ...
        "bottlenecks": [
            {"layer": b.layer, "type": b.type, "severity": b.severity.value,
             "description": b.description, "action": b.action}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("api.main:app", host=settings.DASHBOARD_HOST, port=settings.DASHBOARD_PORT, reload=True)